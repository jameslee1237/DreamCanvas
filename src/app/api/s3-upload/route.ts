...
async function deleteFileFromS3(fileName: string): Promise<string> {
  const params = {
    Bucket: process.env.NEXT_PUBLIC_IAM_BUCKET_NAME ?? "",
    Key: fileName,
  };
...
export async function POST(req: NextRequest) {
  try {
    ...
    const deleteFile = formData.get("delete") as string | null;
    if (deleteFile) {
        const fileName = await deleteFileFromS3(deleteFile);
